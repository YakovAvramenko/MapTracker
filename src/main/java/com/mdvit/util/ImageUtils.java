...
public class ImageUtils {
	...
	public static BufferedImage tilt(BufferedImage image, double angle) {
		...
		int neww = (int) Math.floor(w * cos + h * sin), newh = (int) Math
				.floor(h * cos + w * sin);
		//cant use in Tomcat AS, will HeadLess Exception raise
	...
	public static GraphicsConfiguration getDefaultConfiguration() {
		...
		GraphicsDevice gd = ge.getDefaultScreenDevice();//booom
		return gd.getDefaultConfiguration();
	}

	// http://www.exampledepot.com/egs/java.awt.image/Image2Buf.html
	// An Image object cannot be converted to a BufferedImage object.
	// The closest equivalent is to create a buffered image and then draw the
	// image on the buffered image.
